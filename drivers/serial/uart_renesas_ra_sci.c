...
static void uart_ra_sci_eri_isr(const struct device *dev)
{
	...
	/* Always clear the SCI error flags and the pending IRQ; otherwise a single
	 * RX error with no user callback set (e.g. during uart_configure()) leaves
	 * IELSR.IR asserted and this ISR re-triggers forever. */
